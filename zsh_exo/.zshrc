...
# Configuración inteligente para Wayland (Sway) y X11 (i3)
if [ "$XDG_SESSION_TYPE" = "wayland" ]; then
    # Configuraciones para SWAY
    export MOZ_ENABLE_WAYLAND=1
    export QT_QPA_PLATFORM=wayland
    export XDG_CURRENT_DESKTOP=sway
else
    # Configuraciones para i3
...
gemini() {
  ...
  if [[ -z "$GEMINI_API_KEY" ]]; then
    echo "Error: GEMINI_API_KEY no está configurada. Revisa ~/.zsh_secrets"
    return 1
  fi
